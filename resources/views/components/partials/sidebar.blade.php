<aside class="w-full bg-transparent text-slate-300 flex flex-col min-h-screen">
    ...
    <div class="px-5 py-5 border-b border-white/10">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                @if (request()->is('admin*'))
                    <span
                        class="text-[10px] font-bold uppercase tracking-wider bg-indigo-400/20 text-indigo-300 border border-indigo-400/30 px-2 py-0.5 rounded-md">Admin</span>
                @elseif(request()->is('dokter*'))
                    <span
                        class="text-[10px] font-bold uppercase tracking-wider bg-purple-400/20 text-purple-300 border border-purple-400/30 px-2 py-0.5 rounded-md">Dokter</span>
                @elseif(request()->is('pasien*'))
                    <span
                        class="text-[10px] font-bold uppercase tracking-wider bg-amber-400/20 text-amber-300 border border-amber-400/30 px-2 py-0.5 rounded-md">Pasien</span>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-4">
        ...
        @php
            ...
            $active =
                'bg-gradient-to-r from-white/20 to-white/5 text-white font-semibold border border-indigo-400 border-2';
        @endphp
        ...
        @if (request()->is('admin*'))
            ...
            <div class="space-y-1">
                ...
                {{-- ── Fitur 6: Verifikasi Pembayaran ── --}}
                <a href="{{ route('admin.pembayaran.index') }}"
                    class="{{ $baseLink }} {{ request()->routeIs('admin.pembayaran.*') ? $active : $inactive }}">
                    <i class="fas fa-money-check-dollar w-4 text-center"></i> Verifikasi Pembayaran
                </a>
            </div>
        @endif
        ...
        @if (request()->is('pasien*'))
            <p class="text-xs uppercase tracking-widest text-indigo-400 px-3 mb-3">Menu Pasien</p>
            <div class="space-y-1">
                <a href="{{ route('pasien.dashboard') }}"
                    class="{{ $baseLink }} {{ request()->routeIs('pasien.dashboard') ? $active : $inactive }}">
                    <i class="fas fa-house-medical w-4 text-center"></i> Dashboard
                </a>
                ...
                <a href="{{ route('pasien.riwayat.index') }}"
                    ...
                    <i class="fas fa-clock-rotate-left w-4 text-center"></i> Riwayat Pendaftaran
                </a>
                {{-- ── Fitur 6: Pembayaran ── --}}
                <a href="{{ route('pasien.pembayaran.index') }}"
                    class="{{ $baseLink }} {{ request()->routeIs('pasien.pembayaran.*') ? $active : $inactive }}">
                    <i class="fas fa-receipt w-4 text-center"></i> Pembayaran
                </a>
            </div>
        @endif
        ...
        @if (request()->is('dokter*'))
            <p class="text-xs uppercase tracking-widest text-indigo-400 px-3 mb-3">Menu Dokter</p>
            <div class="space-y-1">
                ...
                <a href="{{ route('dokter.riwayat') }}"
                    ...
                    <i class="fas fa-file-medical w-4 text-center"></i> Riwayat Pasien
                </a>
            </div>
        @endif
    ...
    <div class="p-4 border-t border-white/10">
        <form action="{{ route('logout') }}" method="POST" class="form-confirm" data-title="Keluar dari akun?"
            data-text="Anda akan keluar dari sistem." data-icon="question" data-confirm="Ya, keluar"
            data-cancel="Batal">
            @csrf
            <button type="submit"
